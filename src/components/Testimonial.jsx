...
// PrebuiltUI: tailwindcss css components build sẵn
const Testimonial = () => {
                ...
                {dummyTestimonialData.map((testimonial, index) => (
                    <div key={index} className='p-8 m-4 max-w-xs rounded-lg bg-[#FDFDFE] shadow-lg border border-gray-100 hover:-translate-y-1 transition duration-300 cursor-pointer'>
                        <div className="flex items-center gap-1">
                            {/* index luôn là 0,1,2,3,4 chỉ có testimonial.rating
                            là thay đổi do người dùng
                            vd: index = 4 < rating = 5 thì 
                            số sao sẽ sáng hết 
                            index = 4 > rating = 3 thì
                            số sao sẽ chỉ sáng 3 cái */}
